...
import Task_7.T2.controller.HotelController;
import Task_7.T2.exceptions.*;
import Task_7.T2.factory.HotelMenuFactory;
import Task_7.T2.model.*;
...
public class HotelUI {
    ...
    private static final String CONFIG_FILENAME = "hotel.properties";
    ...
    private HotelUI() {
        ...
        // Если состояние не загружено, создаем новый контроллер
        if (controller == null) {
    ...
    private void loadConfiguration() {
        ...
        try (FileInputStream fis = new FileInputStream(CONFIG_FILENAME)) {
            config.load(fis);
            System.out.println("Конфигурация загружена из " + CONFIG_FILENAME);
        } catch (IOException e) {
    ...
    private void loadSavedState() {
        try {
            this.controller = HotelController.loadState(STATE_FILENAME);
            if (config != null) {
                controller.loadConfiguration(config);
            }
            System.out.println("Состояние программы загружено из " + STATE_FILENAME);
        } catch (HotelManagementException e) {
            System.out.println("Не удалось загрузить сохраненное состояние: " + e.getMessage());
            System.out.println("Будет создано новое состояние.");
        }
    }
    ...
    public void showConfiguration() {
        ...
        System.out.println("3. Сохранить состояние программы");
        System.out.println("4. Загрузить состояние программы");
        System.out.println("0. Назад");

        int choice = readIntInputWithValidation("Выберите параметр для изменения (0-4): ", 0, 4);
        switch (choice) {
            case 0:
                return;
            case 1:
    ...
    private void saveState() {
        try {
            String filename = readNonEmptyString("Введите имя файла для сохранения (по умолчанию " + STATE_FILENAME + "): ");
    ...
    private void loadState() {
        try {
            String filename = readNonEmptyString("Введите имя файла для загрузки (по умолчанию " + STATE_FILENAME + "): ");
            if (filename.trim().isEmpty()) {
                filename = STATE_FILENAME;
            }
            HotelController newController = HotelController.loadState(filename);
            if (config != null) {
                newController.loadConfiguration(config);
            }
